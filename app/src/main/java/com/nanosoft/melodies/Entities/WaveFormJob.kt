...
package com.nanosoft.melodies.Entities
...
class WaveFormJob(private val FilePath: String) : Job(Params(PRIORITY).setDelayMs(5000).persist()) {
    ...
    override fun onRun() {
        ...
        if (file.exists() || file.length() > 0) {
            ...
            try {
                var mSoundFile = com.nanosoft.melodies.SoundEditor.CheapSoundFile.create(mFile.absolutePath, null)
                if (mSoundFile != null) {
                    mSoundFile.computeDoublesForAllZoomLevels()
                    mSoundFile.SaveCache(FilePath)
                    EventBus.getDefault().post(com.nanosoft.melodies.Entities.WaveFormCompletionEvent(FilePath))
                }
            } catch (e: Exception) {
            }
